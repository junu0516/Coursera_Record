...
public class EarthquakeCityMap extends PApplet {
	...
	public void setup() {		
		...
		if (offline) {
			...
			map = new UnfoldingMap(this, 200, 50, 650, 550, new Microsoft.HybridProvider());
			// IF YOU WANT TO TEST WITH A LOCAL FILE, uncomment the next line
		    //earthquakesURL = "2.5_week.atom";
			//tester : test1.atom, test2.atom
			//new Microsoft.HybridProvider()
			//new Google.GoogleMapProvider()
			//Google only allows to access twice a day, so it's not recommended. To check whether our codes are right, more than two chances are needed:)
		}
	    ...
	    printQuakes();
	    System.out.println("Number of Land : "+numL);
	    System.out.println("Number of Ocean : "+numO);	
	...
	private void addKey() {	
		...
		triangle(42.5f,132.5f,50f,118.5f,57.5f,132.5f);		
		...
		for(Marker cm : countryMarkers) {
			...
			for(Marker m : quakeMarkers) {
				if(m instanceof EarthquakeMarker) {
					...
					if(m.getProperty("country").equals(country)) {
						quakeCounter++;
					}
				}
			}
			if(quakeCounter>=1) {
				System.out.println("Country : "+cm.getProperty("name")+", quakeCounter : "+quakeCounter);
			}
		}	
