...
public abstract class Graph {
	...
	public List<Integer> degreeSequence() {
		...
		//degree : sum of in-degree and out-degree of certain vertex
		//in-degree and out-degree : number of edges from certain vertex
		List<Integer> degreeSequence = new ArrayList<Integer>();
		//numVertices needed
		for(int i=0;i<numVertices;i++) {
		//get Sum of degree for both inNeighbors and outNeighbors list from vertex at index i
		List<Integer> inNeighbors = getInNeighbors(i);
		List<Integer> outNeighbors = getNeighbors(i);
		int inDegree = inNeighbors.size();
		int outDegree = outNeighbors.size();
		degreeSequence.add(inDegree+outDegree);
		}
		
		//Use insertion sort for sorting from the largest to the smallest
		int currIndex;
		for(int i=1;i<degreeSequence.size();i++) {
			currIndex = i;
			while(currIndex>0&&degreeSequence.get(currIndex)>degreeSequence.get(currIndex-1)) {
				//if value at currIndex is larger than value at currIndex-1, swap those values
				int temp = degreeSequence.get(currIndex);
				degreeSequence.set(currIndex, degreeSequence.get(currIndex-1));
				degreeSequence.set(currIndex-1, temp);
				currIndex--;
			}
		}
